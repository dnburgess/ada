...
function ecpt_process_data() {
	...
	if($ecpt_post) // if data is being sent
	{
		if( isset( $_POST['ecpt-action'] ) && $_POST['ecpt-action'] == 'add-post-type' && wp_verify_nonce($_POST['ecpt-cpt-nonce'], 'ecpt_add_post_type_nonce') ) {
			...
			if( isset( $_POST['options-icon'] ) && $_POST['options-icon'] != '') { $icon = $_POST['options-icon']; } else { $icon = '/wp-content/themes/ada/assets/plugins/easy-content-types/includes/images/icon.png'; }
		...
		*************************************/
		...
		if( isset( $_POST['ecpt-action'] ) && $_POST['ecpt-action'] == 'update-post-type' && wp_verify_nonce($_POST['ecpt-edit-cpt-nonce'], 'ecpt_edit_cpt_nonce') ) {
			...
			if( isset( $_POST['posttype-menu-icon'] ) && $_POST['posttype-menu-icon'] != '') {
				...
				$icon = 'wp-content/themes/ada/assets/plugins/easy-content-types/includes/images/icon.png';
			}
	...
	*************************************/
	...
	if($ecpt_get) // if data is being sent
	{
		...
		if( isset( $_GET['ecpt-action'] ) && $_GET['ecpt-action'] == 'duplicate' && isset($_GET['metabox-id'] ) ) {
			...
			$duplicate_metabox = $wpdb->query( $wpdb->prepare( "INSERT INTO " . esc_sql( $ecpt_db_meta_name ) . " (`name`,`nicename`,`page`,`context`,`priority`) SELECT `name`,`nicename`,`page`,`context`,`priority` FROM " . $ecpt_db_meta_name . " WHERE `id`='" . absint( $_GET['metabox-id'] ) . "';" , '') );

			// grab the name and ID of the new box
			$new_meta_box = $wpdb->get_results( $wpdb->prepare( "SELECT `name`,`id` FROM " . esc_sql( $ecpt_db_meta_name ) . " ORDER BY `id` DESC LIMIT 1", '') );
			...
			$fields = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . esc_sql( $ecpt_db_meta_fields_name ) . " WHERE `parent`='" . esc_sql( sanitize_text_field( $_GET['metabox-name'] ) ) . "';") );
			if($fields) :
				//print_r($fields); die();
				foreach($fields as $field) :
					$field_parent = $new_box_name;

					$field_id = $wpdb->query( $wpdb->prepare( "INSERT INTO " . esc_sql( $ecpt_db_meta_fields_name ) . " SET
		...
		if( isset( $_GET['ecpt-action'] ) && $_GET['ecpt-action'] == 'delete-field' && isset($_GET['field-id'] ) ) {
			$remove = $wpdb->query( $wpdb->prepare( "DELETE FROM " . esc_sql( $ecpt_db_meta_fields_name ) . " WHERE `id`='" .esc_sql( absint( $_GET['field-id'] ) ) . "';", '') );
